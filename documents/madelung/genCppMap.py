...
#   in the current shell, meaning from the last noble gas upwards, not just
#   non-full shells
import re
...
vdwRadiiMatcher = re.compile(
    r"::(?P<elementName>[A-Z][a-z]?)\,\s(?P<number>[0-9](\.[0-9]+)?)")
vdwRadiiMap = {}
...
with open("vdwradii.txt", "r") as inputFile:
    ...
    for match in vdwRadiiMatcher.finditer(fullText):
        vdwRadiiMap[str(match.group("elementName"))
...
def textRepresentMadelung(madelungTupleList):
    ...
    if highestFullyOccupiedIndex == -1:  # no noble gas reached that we can reduce the representation to
        for nlOccupationTuple in madelungTupleList:
            printstr += str(nlOccupationTuple[0]) + lLetter[nlOccupationTuple[1]] + str(
                nlOccupationTuple[2]) + " "
    else:
...
if False:
    for c, elementName in enumerate(elementNameList):
        ...
        printstr = elementName + \
            " (" + str(Z) + "): " + textRepresentMadelung(madel)
