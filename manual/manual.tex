...
If you want to work with just the Python bindings, those are available as a
...
\begin{Verbatim}
conan install scine_molassembler@conan/stable
\end{Verbatim}

If a prebuilt package is not available, you can make a local build. Start by
cloning the remote repository into a folder and enter it. Then run:
...
If you want conan to build the python bindings as well, add the flag \texttt{-o
scine\_molassembler:python=True} to the \texttt{conan install} step.

Conan package builds default to the lowest common denominator instruction set
architecture, so once again, the library will not be as fast as if you had
compiled it fully yourself.
