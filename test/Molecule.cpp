...
BOOST_AUTO_TEST_CASE(AtomEnvironmentHashesDoNotCollide, *boost::unit_test::label("Molassembler")) {
  auto bitmaskTuple = std::make_tuple(AtomEnvironmentComponents::All);
...
#ifndef NDEBUG
    // Do only one of these in debug builds
    break;
#endif
  }
}
...
BOOST_AUTO_TEST_CASE(Periodic2d, *boost::unit_test::label("Molassembler")) {
  ...
  BOOST_CHECK_EQUAL(interpretedMol.V(), 8);
  BOOST_CHECK_EQUAL(interpretedMol.E(), 16);
...
BOOST_AUTO_TEST_CASE(ShapeClassification, *boost::unit_test::label("Molassembler")) {
  ...
  const auto interconnected = IO::read("shape_classification/interconnected.mol");
  ...
  const auto trigbipy = IO::read("shape_classification/trig_bipy.mol");
