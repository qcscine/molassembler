...
bool JsonSerialization::base64EqualMolecules(const std::string& stringA, const std::string& stringB, BinaryFormat binaryFormat) {
  ...
  for(unsigned int iMol = 0; iMol < stringListA.size(); ++iMol) {
    ...
    if(!equalVersions(versionA, versionB)) {
      std::cerr << "Warning: Graph comparison with graphs generated by different versions of Molassembler!" <<  std::endl;
    }
...
bool JsonSerialization::equalDecisionLists(const std::string& listStringA, const std::string& listStringB) {
  std::vector<std::string> stringListA = splitBase64StringIntoMoleculeStrings(listStringA);
  std::vector<std::string> stringListB = splitBase64StringIntoMoleculeStrings(listStringB);
  if(stringListA.size() != stringListB.size())
    return false;
  for(unsigned int iMol = 0; iMol < stringListA.size(); ++iMol) {
    const std::vector<std::tuple<int, int, int, int> > listA = unpackDecisionListString(stringListA[iMol]);
    const std::vector<std::tuple<int, int, int, int> > listB = unpackDecisionListString(stringListB[iMol]);
