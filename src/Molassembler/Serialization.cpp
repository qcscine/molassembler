...
Molecule deserialize(const nlohmann::json& m) {
   ...
   * are made). As long as we do not increment the version past major = 3,
   * though, this algorithm does not require any changes.
   * For now all versions <= 3 are consistent.
   */
  // Ensure the future hasn't come yet
  static_assert(
    Version::major <= 3,
    "Incrementing the major version past 3 requires that you look at JSON deserialization!"
  );
